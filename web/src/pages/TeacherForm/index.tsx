...
function TeacherForm() {
  ...
  const [scheduleItemList, setScheduleItemList] = useState([
    { week_day: 0, from: '', to: '' }
  ]);
  ...
  function updateScheduleItemValue(position: number, field: string, value: string) {
    const updatedList = scheduleItemList.map((scheduleItem, index) => {
      if (index === position) return {...scheduleItem, [field]: value};
      return scheduleItem;
    })

    setScheduleItemList(updatedList);
  }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <fieldset>
            ...
            {scheduleItemList.map((scheduleItem, index) => (
              <div
                key={`${index}_${scheduleItem.week_day}`}
